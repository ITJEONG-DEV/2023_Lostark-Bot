...
def make_island_box(island: str, rewards: []):
    width, height = 1024 - icon_size[0] * 2, icon_size[1]
    island_box = Image.new('RGBA', (width, height))

    # island
    island_image = get_image(f'data/island', island)
    island_image = island_image.resize(icon_size)

    island_box.paste(island_image, (0, 0), island_image)

    # text
    drawable_image = ImageDraw.Draw(island_box)
    w, h = drawable_image.textsize(island, font=island_font)
    x, y = icon_size[0] + icon_gap * 4, (height - h) / 2
    drawable_image.text((x, y), island, fill=font_color, font=island_font)

    # reward
    rewards_box = make_rewards_box(rewards)
    start_x, start_y = int(width - icon_size[0] * len(rewards) - icon_gap * (len(rewards) - 1)), 0
...
def make_island_content(island_rewards_infoes: [[]], time_text):
    width, height = 1024 - icon_size[0] * 2, icon_size[1] * 3 + content_gap * 2 + icon_size[1] * 2
    ...
    start_x, start_y = 0, icon_size[1] * 2
...
def make_daily_adventure_island(island_rewards_infoes: [[]], date_text):
    ...
    if len(island_rewards_infoes) == 6:
        daily_adventure_island_content = Image.new('RGBA', (window_size[0], window_size[1]*2), background_color)
        ...
        drawable_image.text((x, y), date_text, fill=font_color, font=title_font)

        island_content_1 = make_island_content(island_rewards_infoes[:3], "09:00/11:00/13:00")
        start_x, start_y = icon_size[0], icon_size[1] + content_gap
        ...
        start_x, start_y = icon_size[0], icon_size[1] * 7 + content_gap * 4
        daily_adventure_island_content.paste(island_content_2, (start_x, start_y), island_content_2)

    else:
        daily_adventure_island_content = Image.new('RGBA', window_size, background_color)
...
if __name__ == "__main__":
    ...
    make_daily_adventure_island([
        ["스노우팡 아일랜드", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "실링"]],
        ["기회의 섬", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "기회의 섬의 마음", "강태공의 주머니", "해적 주화", "실링"]],
        ["스노우팡 아일랜드", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "해적 주화", "실링"]]
    ], "2023-03-11 모험섬 일정")\
        .show()
        # .save("D:/평일_테스트1.png")
    ...
    make_daily_adventure_island([
        ["스노우팡 아일랜드", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "해적 주화", "실링"]],
        ["기회의 섬", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "강태공의 주머니", "해적 주화", "실링"]],
        ["스노우팡 아일랜드", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "해적 주화", "실링"]],
        ["스노우팡 아일랜드", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "해적 주화", "실링"]],
        ["기회의 섬", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "강태공의 주머니", "해적 주화", "실링"]],
        ["스노우팡 아일랜드", ["전설 ~ 고급 카드 팩", "영혼의 잎사귀", "인연의 돌", "해적 주화", "실링"]]
